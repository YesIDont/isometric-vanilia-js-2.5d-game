...
P.prototype.fill = function (ctx, color, level) {
    var points = this.points;
    var i = points.length;

    ctx.save();
    ctx.fillStyle = color;
    ctx.translate(this.pos.x, this.pos.y + level);
    ctx.beginPath();
...
P.prototype.stroke = function ( ctx, color, level ) {
    var points = this.points;
    var i = points.length;
    ...
    ctx.lineWidth = 1;
    ctx.beginPath();
    ctx.moveTo(this.pos.x + points[0].x, this.pos.y + points[0].y + level);
    while (i--) ctx.lineTo(this.pos.x + points[i].x, this.pos.y + points[i].y + level);
    ctx.closePath();
    ctx.stroke();
    ctx.restore();
};
