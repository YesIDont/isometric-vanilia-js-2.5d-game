...
createCharacter.prototype.startPosition = function(xS, yS, zS, tileX, tileY) {
  this.x = xS;
  this.y = yS;
  this.z = zS;
  this.lastTileStoodOn = [tileX, tileY];
};
          ...
          if(c >= 0 && c < m.tiles[r].length) {
            ...
            if(m.tiles[r][c].x > cm.area.points.topLeft.x &&
              ...
              m.tiles[r][c].y + m.tileHeight < cm.area.points.bottomRight.y) {
                ...
                if( m.tiles[r][c].z > p.z + p.maxStepWithoutJump || m.tiles[r][c].z < p.z - p.maxStepWithoutJump ) {
                  m.fillOneTileBase(ctx, "rgba(255, 0, 0, 0.4)", r, c);
                }
                var player = cm.base.point;
          ...
          if(c >= 0 && c < m.tiles[r].length) {
            ...
            if(m.tiles[r][c].x > cm.area.points.topLeft.x &&
              ...
              m.tiles[r][c].y + m.tileHeight < cm.area.points.bottomRight.y) {
                ...
                var t = m.tiles[r][c];
                // continue to SAT collision test if tile is higher than player's ability to step on tile
                // making him unable to move forward
                ...
                // if player is on the same level as tile or stepped on tile lover
                } else {
...
createCharacter.prototype.move = function(m, p) {
  ...
  if ( p.z < l.z ) {
    p.isInTheAir = true;
    p.zV = p.zV < 3 ? p.zV += 1 : 3;
    p.jumpDelayCounter = p.jumpDelay;

  } else {
    p.zV = 0;
    p.isInTheAir = false;
    p.jumpDelayCounter = p.jumpDelayCounter > 0 ? p.jumpDelayCounter -= 1 : 0;
  }


  // Player is holding spacebar (jump) key
  if ( 32 in keysDown && !p.isInTheAir && p.jumpDelayCounter === 0 ) {
    p.zV = -p.jumpHeight;
  } 
  ...
  if( p.zV < 0 ) { // if player is falling
    p.z += p.zV;
    ...
    if ( p.z < l.z) {
      ...
      p.z = l.z; // stops player from falling below tile he is falling on
